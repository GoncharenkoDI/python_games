...
def show_player_info(message: str, has_sword: bool, has_princess: bool, has_key: bool):
    ...
    print("У вас є: ", end="")
    if has_sword:
        print(f" {SWORD_ICO} - меч", end="")
    if has_key:
        print(f" {KEY_ICO} - ключ", end="")
    if has_princess:
        print(f" {PRINCESS_ICO} - принцеса", end="")
    print()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if not inside_forest(player):  # aВийшов з лісу
            break

        if (
            cell == PRINCESS_ICO and not has_princess
        ):  # був на місті принцеси і не забрав її
